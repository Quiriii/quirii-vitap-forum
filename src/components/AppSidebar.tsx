  ...
  return <Sidebar className="border-r border-sidebar-border">
      <SidebarHeader className="p-4 border-b border-sidebar-border">
        <div className="text-sidebar-foreground">
          <h2 className="text-lg font-bold">Quirii</h2>
      ...
      <SidebarContent>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {LADIES_HOSTELS.map(hostel => <SidebarMenuItem key={hostel}>
                  <SidebarMenuButton asChild disabled={!canAccessHostel(hostel)}>
                    <NavLink to={`/category/${hostel}`} className={canAccessHostel(hostel) ? getNavClassName : 'opacity-50 cursor-not-allowed'}>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {MENS_HOSTELS.map(hostel => <SidebarMenuItem key={hostel}>
                  <SidebarMenuButton asChild disabled={!canAccessHostel(hostel)}>
                    <NavLink to={`/category/${hostel}`} className={canAccessHostel(hostel) ? getNavClassName : 'opacity-50 cursor-not-allowed'}>
                      <Building2 className="mr-2 h-4 w-4" />
                      <span>{hostel}</span>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {COMMON_SECTIONS.map(section => <SidebarMenuItem key={section}>
                  <SidebarMenuButton asChild>
